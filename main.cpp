...
HRESULT OnRender()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		wsprintf(szText, TEXT("%d年 %d月"), dwYear, dwMonth);
		...
		const TCHAR szYoubi[] = TEXT("日月火水木金土");
		...
		for (int i = 0; i < 7; i++)
		{
			...
			if (szYoubi[i] == TEXT('土')) pBrush = m_pBlueBrush;
			else if (szYoubi[i] == TEXT('日')) pBrush = m_pRedBrush;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInst, LPSTR pCmdLine, int nCmdShow)
{
	...
	m_hwnd = CreateWindow(szClassName, TEXT("カレンダー"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, 0, 0, hInstance, 0);
